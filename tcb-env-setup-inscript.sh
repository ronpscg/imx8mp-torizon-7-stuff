...
# alias workaround
# Ron says: if the script is sourced inside another script, torizoncore-builder will not work as it is an alias
#           so define and export a function instead.
#	    This is *very* bash specific, and it will not be tested with other shells (specifically tested with bash version > 5.0)
if [ "$(basename ${BASH_SOURCE[1]})" = "build-me.sh" ] ; then
	echo "Called from our script - unaliasing and making torizoncore-builder a function"
	torizoncorebuilderworkaroundcmd=$(echo docker run --rm -it "$volumes" -v "$(pwd)":/workdir -v "$storage":/storage -v /var/run/docker.sock:/var/run/docker.sock "$network" $(tcb_dynamic_params) "$*" torizon/torizoncore-builder:"$chosen_tag")
	torizoncore-builder() {
		$torizoncorebuilderworkaroundcmd $@
	}
	export -f torizoncore-builder
fi
# end of alias workaround
